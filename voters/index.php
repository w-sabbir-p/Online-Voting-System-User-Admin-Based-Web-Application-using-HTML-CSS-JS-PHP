        ...
        <?php 
            ...
            if($totalActiveElections > 0) {
                while($data = mysqli_fetch_assoc($fetchingActiveElections)) {
                    ...
                    // Check if the voter has already voted in this election topic
                    ...
                    $hasVoted = mysqli_num_rows($checkIfVoteCasted) > 0;

                    // Determine if the vote button should be disabled
                    $disableVoteButton = $hasVoted ? "disabled" : "";

                    // Fetch the candidate the voter has voted for in this election topic
                    ...
                    <table class="table">
                        ...
                        <?php 
                            ...
                            while($candidateData = mysqli_fetch_assoc($fetchingCandidates)) {
                                ...
                                // Determine if the current candidate is the one the voter has voted for
                                $votedForThisCandidate = ($hasVoted && $voteCastedToCandidate == $candidate_id);
                        ?>
                                <tr>
                                    <td> <img src="<?php echo $candidate_photo; ?>" class="candidate_photo" style="width: 60px; height: 60px;"> </td>
                                    <td><?php echo "<b>" . $candidateData['candidate_name'] . "</b><br />" . $candidateData['candidate_details']; ?></td>
                                    <td><?php echo $totalVotes; ?></td>
                                    <td>
                                        <?php if ($votedForThisCandidate) { ?>
                                            <img src="../assets/images/vote.png" width="50px;">
                                        <?php } else { ?>
                                            <button class="btn btn-md btn-success" onclick="CastVote(<?php echo $election_id; ?>, <?php echo $candidate_id; ?>, <?php echo $_SESSION['user_id']; ?>)" <?php echo $disableVoteButton; ?>> Vote </button>
                                        <?php } ?>
                                    </td>
                                </tr>
                        <?php
